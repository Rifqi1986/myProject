...
exports.up = function(knex) {
    return knex.schema.createTable('treatment', function (table) {
        ...
        table.integer('dataPetugas_id').unsigned().notNullable().references('id').inTable('dataPetugas').onDelete('CASCADE').onUpdate('CASCADE');
        table.integer('dataObat_id').unsigned().notNullable().references('id').inTable('dataObat').onDelete('CASCADE').onUpdate('CASCADE');
